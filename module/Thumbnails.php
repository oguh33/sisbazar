...
class Thumbnails
{
    ...
    {
        preg_match('/(upload\/pecas\/)(.*?)(\..*?)$/is', $file, $match);
        return PATH.$match[1].'small_'.$match[2].$match[3];
    }
    public function getPathFileOrigin($file)
    {
        preg_match('/(upload\/pecas\/)(.*?)(\..*?)$/is', $file, $match);
        return PATH.$match[1].$match[2].$match[3];
    }
    public function getPathFileOriginComprovante($file)
    {

        preg_match('/(upload\/comprovante\/)(.*?)(\..*?)$/is', $file, $match);
        return PATH.$match[1].$match[2].$match[3];
    }
